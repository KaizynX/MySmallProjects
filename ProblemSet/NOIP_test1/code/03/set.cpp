 ...
 * @LastEditTime: 2020-08-25 19:23:34
 */
#include <bits/stdc++.h>
...
void my_add(int x, int k) {
  q[x].insert(k);
  seg.update(x, *q[x].rbegin());
}

void my_del(int x, int k) {
  q[x].erase(q[x].find(k));
  seg.update(x, q[x].size() ? *q[x].rbegin() : -INF);
}
...
signed main() {
  ...
  seg.build(m);
  for (int i = 1; i <= n; ++i) {
    if (i-L >= 0) my_add(presum[i-L], dp[i-L]);
    if (i-R-1 >= 0) my_del(presum[i-R-1], dp[i-R-1]);
