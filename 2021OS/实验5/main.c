...
#define DELIM           "/"
#define FOLDER_COLOR    "\e[1;36m"  // 加粗 + 绿色
...
int my_rmdir(char **args) {
    ...
    for (int i = 1; args[i] != NULL; ++i) {
        ...
        fcb *tmp = (fcb *)(BLOCK_SIZE * (dir->first) + fs_head);
        for(int j=0;j<BLOCK_SIZE/ sizeof(fcb);++j,++tmp){
            if (j<2||tmp->free == 0)continue;
            fprintf(stderr, "rmdir: cannot remove %s: Directory not empty\n", args[i]);
            return 1;
        }
...
void do_ls(int first, char mode) {
    ...
    if (mode == 'n') {
        ...
        printf("%s\t%6s\t%6s\t%10s\t%8s\t%s\n", "attribute", "first", "size", "date", "time", "name");
        for (int i = 0; i < length / sizeof(fcb); ++i, ++root) {
            ...
            get_fullname(fullname, root);
            printf("%9d\t%6d\t%6ld\t%s\t%s\t", root->attribute, root->first, root->length, date, time);
            if (root->attribute == 0) {
                printf("%s", FOLDER_COLOR);
                printf("%s\n", fullname);
                printf("%s", DEFAULT_COLOR);
...
int do_create(const char *parpath, const char *filename) {
    ...
    if (token != NULL) {
        ...
        fprintf(stderr, "create: No suffix %s\n", parpath);
        return -1;
    }
...
int set_free(unsigned short first, unsigned short length, int mode) {
    ...
    if (mode == 1) {
        ...
        while (1) {
            ...
            fat1->id = FREE;
            if(nxt==END)break;
            fat0 = (fat *)(fs_head + BLOCK_SIZE + nxt*sizeof(fat));
            fat1 = (fat *)(fs_head + BLOCK_SIZE*3+nxt*sizeof(fat));
            i=nxt;
        }
    }
    else if (mode == 2) {
...
int my_fat(char **args){
    fat *fat0 = (fat *)(fs_head + BLOCK_SIZE);
    for(int i=0;i<BLOCK_SIZE/sizeof(fat);i++){
        fprintf(stderr, "%04x ",fat0->id);
        fat0++;
...
int cmd_execute(char **args) {
    ...
    for (int i = 0; i < cmd_num_builtins(); ++i) {
        if (strcmp(args[0], builtin_str[i]) == 0)
            return (*built_func[i])(args);
    }

    return cmd_launch(args);                        // 不是自行实现的函数
}
...
char **cmd_split_line(char *line) {
    ...
    if (!tokens) {
        fprintf(stderr, "cmd: allocation cmd\n");
        exit(EXIT_FAILURE);
    }
 ...
 \:\/:/ /:/  / \:\~\:\ \/__/ \:\  \    \:\  \  \/__/ \:\  \ /:/  / \/__/~~/:/  / \:\~\:\ \/__/
  \::/_/:/  /   \:\ \:\__\    \:\  \    \:\  \        \:\  /:/  /        /:/  /   \:\ \:\__\
   \:\/:/  /     \:\ \/__/     \:\  \    \:\  \        \:\/:/  /        /:/  /     \:\ \/__/  
    ...
    do {
        printf("\e[34;1mzkx@19063140 \e[37;1m%s\e[0m ", current_dir);
        printf("\e[33m$\e[0m ");
        line = cmd_read_line();
        args = cmd_split_line(line);
        state = cmd_execute(args);
