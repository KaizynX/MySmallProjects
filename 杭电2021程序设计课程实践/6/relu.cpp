 ...
 * @LastEditTime: 2021-06-07 17:26:13
 */
#include <bits/stdc++.h>
// #define DEBUG
using namespace std;
 ...
 public:
  ...
  void init() {
    ...
    for (auto &i : w)
      for (auto &j : i)
        for (auto &k : j)
          k = 0.9*(rand() * 2.0 / RAND_MAX - 1);  //权重取[-1,1]之间
  }
...
//    }
    ...
    for(int i = 0;i < l3;i++){
        ...
        dz2[i]=da2[i]*gp(z[2][i]);//*s[2][i]*(1-s[2][i]);
        for(int j = 0;j < l2;j++){
    ...
    for(int i = 0;i < l2;i++){
        ...
        dz1[i]=da1[i]*gp(z[1][i]);//*s[1][i]*(1-s[1][i]);
        for(int j = 0;j < l1;j++){
