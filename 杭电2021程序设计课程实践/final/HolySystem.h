 ...
 * @LastEditTime: 2021-06-05 21:52:06
 */
#ifndef BITS
#define BITS
#include <bits/stdc++.h>
...
class Gender {
 public:
  ...
  friend bool operator ==(const Gender &lhs, const Gender &rhs) { return lhs.gender == rhs.gender; }
  bool check() {
    buf = trim(buf);
...
class IdNumber {
 public:
  ...
  friend bool operator ==(const IdNumber &lhs, const IdNumber &rhs) { return lhs.id == rhs.id; }
  bool check() {
    id = trim(buf);
...
class PhoneNumber {
 public:
  ...
  friend bool operator ==(const PhoneNumber &lhs, const PhoneNumber &rhs) { return lhs.number == rhs.number; }
  bool check() {
    number = trim(buf);
...
class NumberPlate {
 public:
  ...
  friend bool operator ==(const NumberPlate &lhs, const NumberPlate &rhs) { return lhs.number == rhs.number; }
  bool check() {
    number = trim(buf);
    if (number.empty()) return throw "输入不能为空", false;
...
class HealthCode {
 public:
  ...
  friend bool operator ==(const HealthCode &lhs, const HealthCode &rhs) { return lhs.color == rhs.color; }
  bool check() {
    buf = trim(buf);
    if (buf.empty()) return throw "输入不能为空", false;
...
class Bool {
 public:
  ...
  friend bool operator ==(const Bool &lhs, const Bool &rhs) { return lhs.tag == rhs.tag; }
  bool check() {
    buf = trim(buf);
    if (buf.empty()) return throw "输入不能为空", false;
...
class Time {
 public:
  friend istream& operator >>(istream &is, Time &t) {
    cout << "格式(year-month-day hour:minute:second) 例如 2021-06-06 01:23:34 " << endl;
    input(is, t);
    return is;
  }
  ...
  friend bool operator ==(const Time &lhs, const Time &rhs) { return !(lhs < rhs || rhs < lhs); }
  friend bool operator !=(const Time &lhs, const Time &rhs) { return lhs < rhs || rhs < lhs; }
  bool check() {
    static char ch;
...
class HolySystem {
 public:
  ...
  vector<BasicInformation> query_name() {
    ...
    for (auto &p : database) if (p.name == name) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_gender() {
    ...
    for (auto &p : database) if (p.gender == gender) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_phone_number() {
    ...
    for (auto &p : database) if (p.phone_number == phone_number) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_id_number() {
    ...
    for (auto &p : database) if (p.id_number == id_number) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_work_place() {
    ...
    for (auto &p : database) if (p.work_place == work_place) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_number_plate() {
    ...
    for (auto &p : database) if (p.number_plate == number_plate) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_reason() {
    ...
    for (auto &p : database) if (p.reason == reason) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_guarantor() {
    ...
    for (auto &p : database) if (p.guarantor == guarantor) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_guarantor_phone_number() {
    ...
    for (auto &p : database) if (p.guarantor_phone_number == guarantor_phone_number) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_is_epidemic_area() {
    ...
    for (auto &p : database) if (p.is_epidemic_area == is_epidemic_area) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_is_unhealthy_symptoms() {
    ...
    for (auto &p : database) if (p.is_unhealthy_symptoms == is_unhealthy_symptoms) vec.emplace_back(p);
    return vec;
  }
  vector<BasicInformation> query_health_code() {
    ...
    for (auto &p : database) if (p.health_code == health_code) vec.emplace_back(p);
    return vec;
  }
