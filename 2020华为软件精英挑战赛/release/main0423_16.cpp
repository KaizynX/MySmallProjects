...
#undef add_ans
	...
	inline void solve(const int *eset, const int &sz, const int &_n) {
		...
		else
		{
			...
			multi_thread_info[2] = { n >>5, 3*n/64 };
			multi_thread_info[3] = { 3 * n / 64,n>>4};

			multi_thread_info[4] = { n>>4, 5*n/64 };
			multi_thread_info[5] = { n*5/64, n*3/32 };

			multi_thread_info[6] = { n*3/32, 7* n / 64 };
			multi_thread_info[7] = { 7* n /64,n >> 3};

			multi_thread_info[8] = { n>>3,39*n/256};
			multi_thread_info[9] = { 39*n/256, n * 23 / 128 };

			multi_thread_info[10] = { n*23/128 , 53*n/256 };
			multi_thread_info[11] = { 53*n/256 , 30 * n / 128 };

			multi_thread_info[12] = { n*30/128, 289/1024};
			multi_thread_info[13] = {289*n/1024,n * 169 / 512 };

			multi_thread_info[14] = { n * 169 / 512,  1019* n / 2048 };
			multi_thread_info[15] = { 1019*n/2048, n };
...
inline void input()
{
	struct stat statbuf;
	register int sz = 0;
	if (stat("./data/test_data2.txt", &statbuf) == 0)sz = statbuf.st_size;
	int fd = open("./data/test_data2.txt", O_RDONLY);
