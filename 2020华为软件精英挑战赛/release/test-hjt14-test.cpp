...
struct Timer{
	...
	void operator()(const string &s){
		double now=clock();
		cerr<<"["<<s<<"] ";
		cerr<<"Interval: "<<(now-pre)/CLOCKS_PER_SEC<<"s | ";
		cerr<<"Total:"<<(now-start)/CLOCKS_PER_SEC<<"s"<<endl;
		pre=now;
	}
}timer;
...
void sort(vector<vector<int>> &ans, const int &n) {
	int sz = ans.size(), pos;
...
vector<vector<int>>& solve(const int *eset, const int &sz, const int &_n){
	...
	for(int i=0;;i++){
		...
		s=bkt::head[bkt::top];if(i%1000==0)orz(s);
