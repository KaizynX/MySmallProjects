...
#define re register
	...
	inline int init(u32 *_c, const int &_n) {
		...
		/* O(sort(n)+n)
		std::iota(b, b+n, 0);
		std::sort(b, b+n, [](const int &i, const int &j) { return c[i] < c[j]; });
		for (re int *i = b, j = 0, *ed = b+n; i < ed; ++i) {
			while (a[j] < c[*i]) ++j;
			c[*i] = j;
		}
		*/
...
inline void bfs_b7();
inline void topo();
inline vector<vector<int>>* solve(const int*, const int &, const int&);

inline void for_dfs(const int &p0) {
	rec[0] = p0; vis[p0] = 1;
	for (const auto &p1 : a[p0]) {
		if (p1 <= s || vis[p1]) continue;
		rec[1] = p1; vis[p1] = 1;
		for (const auto &p2 : a[p1]) {
			if (p2 <= s || vis[p2]) continue;
			rec[2] = p2; vis[p2] = 1;
			for (const auto &p3 : a[p2]) {
				if (p3 == s) { ans[3].emplace_back(rec.begin(), rec.begin()+3); continue; }
				if (p3 < s || vis[p3]) continue;
				rec[3] = p3; vis[p3] = 1;
				for (const auto &p4 : a[p3]) {
					if (p4 == s) { ans[4].emplace_back(rec.begin(), rec.begin()+4); continue; }
					if (p4 < s || vis[p4] || dcnt_b[p4] != dcnt) continue;
					rec[4] = p4; vis[p4] = 1;
					for (const auto &p5 : a[p4]) {
						if (p5 == s) { ans[5].emplace_back(rec.begin(), rec.begin()+5); continue; }
						if (p5 < s || vis[p5] || dcnt_b[p5] != dcnt || dis_b[p5] > 2) continue;
						rec[5] = p5; vis[p5] = 1;
						for (const auto &p6 : a[p5]) {
							if (p6 == s) { ans[6].emplace_back(rec.begin(), rec.begin()+6); continue; }
							if (p6 > s && !vis[p6] && dcnt_b[p6] == dcnt && dis_b[p6] == 1) {
								rec[6] = p6;
								ans[7].emplace_back(rec.begin(), rec.end());
							}
						}
						vis[p5] = 0;
					}
...
inline vector<vector<int>>* solve(const int *eset, const int &sz, const int &_n){
