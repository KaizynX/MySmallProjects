...
#define re register
	...
	inline int init(u32 *_c, const int &_n) {
		...
		for (re int i = 0; i < m; ++i) {
			int &x = a[i];
			ptr = str[i];
			if (x >= 10000000000u)*(ptr++) = (x / 10000000000u) ^ 48u;
			if (x >= 1000000000u) *(ptr++) = (x / 1000000000u % 10u) ^ 48u;
				...
				for (p4 = a_begin[p3->first], ed4 = a[p3->first].end(); p4 != ed4; ++p4) {
					...
					for (p5 = a_begin[p4->first], ed5 = a[p4->first].end(); p5 != ed5; ++p5) {
						...
						for (p6 = a_begin[p5->first], ed6 = a[p5->first].end(); p6 != ed6; ++p6) {
							if (!vis[p6->first] && circle_check(p6)) {
								rec[6] = p6->first;
								add_ans(7);
							}
						}
...
{
	n = 0;
	for (register int i = 3; i <= 7; ++i)n += Solve::ans[i].size() / i;
...
#define write_char(str,ch) *(str++)=(ch)
	if (n >= 10000000u)   write_char(mapped, (n / 10000000u) ^ 48u);
