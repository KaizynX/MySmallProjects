...
namespace Solve{
const int dis1 = 1u<<29;
const int dis2 = 2u<<29;
const int dis3 = 3u<<29;
const int dis4 = 4u<<29;
const int dcnt_b = (1u<<29)-1;

int A[N][51], b[N][51];
int *a_begin[N], *a_end[N], *b_end[N];
int in_deg[N],ot_deg[N];
int dcnt[N]; // dis_b << 29 + dcnt_b
bool vis[N];
Queue q;
int rec[10];
int ans3[3*500000];
	...
	for (p1 = a_begin[p0], ed1 = a_end[p0]; p1 != ed1; ++p1) {
		...
		for (p2 = a_begin[*p1], ed2 = a_end[*p1]; p2 != ed2; ++p2) {
			...
			for (p3 = a_begin[*p2], ed3 = a_end[*p2]; p3 != ed3; ++p3) {
				if (vis[*p3] || (dcnt[*p3]&dcnt_b) != p0) continue;
				rec[3] = *p3; vis[*p3] = 1;
				if (dcnt[*p3] == circle_end) { add_ans(4); }
				for (p4 = a_begin[*p3], ed4 = a_end[*p3]; p4 != ed4; ++p4) {
					if (vis[*p4] || (dcnt[*p4]&dcnt_b) != p0 || dcnt[*p4] >= dis4) continue;
					rec[4] = *p4; vis[*p4] = 1;
