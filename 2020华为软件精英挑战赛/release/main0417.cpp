...
inline void output(const vector<vector<int>>*ans)
{
	n = 0;
	for (register int i = 0; i <= 7; ++i)n += ans[i].size();
	...
	{
		for (register int k = 0; k < ans[i].size(); ++k)
		{
			for (register int j = 0; j < ans[i][k].size(); ++j)
			{
				memcpy(mapped, DISC::str[ans[i][k][j]], DISC::len[ans[i][k][j]]);
				mapped += DISC::len[ans[i][k][j]];
				*(mapped++) = ',';
			}
			*(mapped - 1) = '\n';
		}
