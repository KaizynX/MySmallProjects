...
#define add_ans(len); memcpy(ans_end[len], rec, sizeof(int)*len);ans_end[len]+=len;
	...
	for (p1 = a_begin[p0], ed1 = a_end[p0]; p1 != ed1; ++p1) {
		...
		for (p2 = a_begin[*p1], ed2 = a_end[*p1]; p2 != ed2; ++p2) {
			if (vis[*p2]) continue;
			rec[2] = *p2; vis[*p2] = 1;
			if (dcnt_b[*p2] == p0 && dis_b[*p2] == 1) { add_ans(3); }
			for (p3 = a_begin[*p2], ed3 = a_end[*p2]; p3 != ed3; ++p3) {
				if (vis[*p3]) continue;
				rec[3] = *p3; vis[*p3] = 1;
				if (dcnt_b[*p3] == p0 && dis_b[*p3] == 1) { add_ans(4); }
				for (p4 = a_begin[*p3], ed4 = a_end[*p3]; p4 != ed4; ++p4) {
					if (vis[*p4] || dcnt_b[*p4] != p0) continue;
					rec[4] = *p4; vis[*p4] = 1;
					if (dcnt_b[*p4] == p0 && dis_b[*p4] == 1) { add_ans(5); }
					for (p5 = a_begin[*p4], ed5 = a_end[*p4]; p5 != ed5; ++p5) {
						if (vis[*p5] || dcnt_b[*p5] != p0 || dis_b[*p5] > 2) continue;
						rec[5] = *p5; vis[*p5] = 1;
						if (dcnt_b[*p5] == p0 && dis_b[*p5] == 1) { add_ans(6); }
						for (p6 = a_begin[*p5], ed6 = a_end[*p5]; p6 != ed6; ++p6) {
							if (!vis[*p6] && dcnt_b[*p6] == p0 && dis_b[*p6] == 1) {
								rec[6] = *p6;
								add_ans(7);
							}
						}
