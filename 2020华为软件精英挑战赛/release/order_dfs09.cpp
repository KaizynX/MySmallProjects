...
inline void solve(const int *eset, const int &sz, const int &_n){
	...
	static Info rebuild_part[2];
	rebuild_part[0].start = 0;
	rebuild_part[0].end = n*0.4;
	rebuild_part[1].start = n*0.4;
	rebuild_part[1].end = n;
	for (int part = 0; part < 2; ++part) {
		if (part) rebuild(rebuild_part[part].start);
		for (int i = rebuild_part[part].start; i < rebuild_part[part].end; ++i) if (!vis[i]) {
			vis[i] = 1;
			dcnt++;
			bfs_b7(i);
			for_dfs(i);
...
inline void rebuild(const int &x) {
	for (int i = x; i < n; ++i) if (!vis[i]) {
		a[i] = lower_bound(a[i], a_end[i], x);
		b[i] = lower_bound(b[i], b_end[i], x);
	}
}
