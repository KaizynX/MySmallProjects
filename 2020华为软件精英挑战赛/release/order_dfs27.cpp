...
void ans_sort() {
	{
		while (*qaq == 0xffffffff) { Sleep(1); }
		for (int i = 3; i <= 7; ++i) {
			int *&it = multi_ans[*qaq][i];
			while (*it == 0 && *(it + 1)) {
				for (int j = 0; j < i; ++j) {
	...
	for (int h = 1; h < n; ++h) {
		while (qaq[h] == 0xffffffff) { Sleep(1); }
		// while (qaq[h] == 0xffffffff) { /* fuck something */ }
		for (int i = 3; i <= 7; ++i) {
...
inline void solve(const int *eset, const int &sz, const int &_n){
	...
	point_pool.h = 0;
	timer("",0,1);
	final_ans_end[3] = final_ans[3] = final_ans3;
	final_ans_end[4] = final_ans[4] = final_ans4;
	final_ans_end[5] = final_ans[5] = final_ans5;
	final_ans_end[6] = final_ans[6] = final_ans6;
	final_ans_end[7] = final_ans[7] = final_ans7;
#endif
	n = _n;
...
inline void input(){
	freopen("test_data3.txt","r",stdin);
...
void output(){
	...
	printf("%u\n",Solve::circle_num);
	for (int i = 3; i <= 7; ++i) {
		*Solve::final_ans_end[i] = '\0';
		printf("%s", Solve::final_ans[i]);
	}
}
