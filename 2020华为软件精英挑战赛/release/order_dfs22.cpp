...
inline void dfs6(const int &p0, const int &id) {
...
#define add_ans(len); memcpy(ans_end[len], rec, sizeof(int)*len);ans_end[len]+=len;
	int *p1, *p2, *p3, *p4, *p5, *p6, *ed1, *ed2, *ed3, *ed4, *ed5;
...
inline void dfs7(const int &p0, const int &id) {
	int *dcnt = multi_dcnt[id];
	bool *vis = multi_vis[id];
	int *rec = multi_rec[id];
	int **ans_end = multi_ans_end[id];
...
inline void solve(const int *eset, const int &sz, const int &_n){
	...
	multi_thread_info[2] = { n >>4,n*3/32 };
	multi_thread_info[3] = { n *3 / 32, n>>3 };
	multi_thread_info[4] = { n>>3, n*23/128 };
	multi_thread_info[5] = { n*23/128 , 30*n/128 };
	multi_thread_info[6] = { n*30/128,n * 169 / 512 };
	multi_thread_info[7] = { n * 169 / 512, n };
