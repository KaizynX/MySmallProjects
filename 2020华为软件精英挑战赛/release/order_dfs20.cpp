...
// int multi_ids[MAX_THREAD_COUNT];
...
void multi_thread_solve(const int &id) {
	...
	while (~(st = point_pool.qwq())) {
		ed = min(st+WORK_STEP, n);
		fill(vis+last, vis+st, true);
		for (int i = st; i < ed; ++i) {
			vis[i] = 1;
			bfs_b7(i, id);
			for_dfs(i, id);
...
struct OP{
	bool operator()(const int &a,const int &b){
		return *Solve::multi_ans[a][len] > *Solve::multi_ans[b][len];
	}
};

priority_queue<int, vector<int>, OP> pri_q;

inline void work() { // sort, get sorted answers from (multi_ans) to (final_ans)
	using namespace Solve;
	for (len = 3; len <= 7; ++len) {
		for (int t = 0; t < THREAD_COUNT; ++t)
		if (multi_ans[t][len] != multi_ans_end[t][len])
			pri_q.push(t), *multi_ans_end[t][len] = -1;
		while (!pri_q.empty()) {
			int t = pri_q.top(); pri_q.pop();
			int *&head = multi_ans[t][len];
			int *ptr = head, head_element = *head;
			do{
				ptr += len;
			}while (*ptr == head_element);
			memcpy(final_ans_end[len], head, (char *)ptr - (char *)head);
			final_ans_end[len] += ptr - head;
			head = ptr;
			if(head != multi_ans_end[t][len])
				pri_q.push(t);
		}
	}
}
...
void output(){
	...
	TAT::work();
	printf("%u\n", unsigned(
		(Solve::final_ans_end[3] - Solve::final_ans[3])/3+
		(Solve::final_ans_end[4] - Solve::final_ans[4])/4+
		(Solve::final_ans_end[5] - Solve::final_ans[5])/5+
		(Solve::final_ans_end[6] - Solve::final_ans[6])/6+
		(Solve::final_ans_end[7] - Solve::final_ans[7])/7
	));
	
	for(int len = 3; len <= 7; len++) {
		int *it = Solve::final_ans[len], *ed = Solve::final_ans_end[len];
		while (it != ed)
		for (int j = 0; j < len; ++j)
