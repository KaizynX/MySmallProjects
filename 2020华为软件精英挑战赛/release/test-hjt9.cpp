...
vector<vector<int>> multi_ans[THREAD_COUNT];
...
void dfs(const int &id, const int &s, const int &x){
	...
	vector<vector<int>> &cur_ans=multi_ans[id];

	vis[x]=1;
	for(const auto &p:a[x])
	if(p==s){
		if(rec.size()>=3)cur_ans.push_back(rec);
	}
...
void multi_thread_solve(int id,int s){
	bfs_a3(id, s);
	bfs_b7(id, s);
...
vector<vector<int>> solve(const vector<int> &eset,int n){
	...
	while(order.size()){
		for(int i=0;i<THREAD_COUNT && order.size();++i){
			...
			threads[i]=thread(multi_thread_solve,i,s);
			q.push(s);
			inq[s]=1;
		}
	...
	for(int i=0;i<THREAD_COUNT;++i)if(multi_ans[i].size()){
		ans.insert(ans.end(), multi_ans[i].begin(), multi_ans[i].end());
	}
	adjust(ans);
	return ans;
}
