...
namespace Solve{
int A[N][50], B[N][255];
int *a_begin[N], *a_end[N], *b_begin[N], *b_end[N];
int n;
...
#define rebuild(node1,node2) \
	ed##node2 = a_end[*p##node1]; \
	for (register int *&i = a_begin[*p##node1]; i != ed##node2 && *i < p0; ++i);
	static const int *p1, *p2, *p3, *ed1, *ed2, *ed3, *ed4;
	rec[0] = p0;
	for (p1 = a_begin[p0], ed1 = a_end[p0]; p1 != ed1; ++p1) {
		if (vis[*p1]) continue;
		rec[1] = *p1; vis[*p1] = 1;
		rebuild(1,2);
		for (p2 = a_begin[*p1]; p2 != ed2; ++p2) {
			if (vis[*p2]) continue;
			rec[2] = *p2; vis[*p2] = 1;
			rebuild(2,3);
			for (p3 = a_begin[*p2]; p3 != ed3; ++p3) {
				if (*p3 == p0) { add_ans(3); continue; }
				if (vis[*p3]) continue;
				rec[3] = *p3; vis[*p3] = 1;
				rebuild(3,4);
				for (register const int *p4 = a_begin[*p3], *const ed4 = a_end[*p3]; p4 != ed4; ++p4) {
					if (*p4 == p0) { add_ans(4); continue; }
					if (vis[*p4] || dcnt_b[*p4] != dcnt) continue;
					rec[4] = *p4; vis[*p4] = 1;
					for (register const int *p5 = a_begin[*p4], *const ed5 = a_end[*p4]; p5 != ed5; ++p5) {
						if (*p5 == p0) { add_ans(5); continue; }
						if (vis[*p5] || dcnt_b[*p5] != dcnt || dis_b[*p5] > 2) continue;
						rec[5] = *p5; vis[*p5] = 1;
						for (register const int *p6 = a_begin[*p5], *const ed6 = a_end[*p5]; p6 != ed6; ++p6) {
...
inline void bfs_b7(const int &s){
	...
	while(!q.empty()){
		...
		for (int *p = b_begin[x], *ed = b_end[x]; p != ed; ++p)
