...
#define re register
	...
	inline void disc(int s, int t) {
		for (u32 *it = c+s, *ed = c+t; it < ed; ++it) {
	...
	inline int init(u32 *_c, const int &_n) {
		...
		for (re int i = 0; i < THREAD_COUNT; ++i) {
			threads[i] = thread(disc, info[i].start, info[i].end);
		}
		for (re int i = 0; i < THREAD_COUNT; ++i) {
...
void multi_thread_rotate(int s, int t) {
	for (int i = s; i < t; ++i) {
...
void adjust(){ //把环里的最小值转到最前并排序
	...
	info[THREAD_COUNT-1].end = n;
	for (int i = 0; i < THREAD_COUNT; ++i) {
		threads[i] = thread(multi_thread_rotate, info[i].start, info[i].end);
	}
	for (int i = 0; i < THREAD_COUNT; ++i) {
		if (threads[i].joinable()) threads[i].join();
	}
