...
inline void solve() {
  ...
  for (int i = 0, k; i < n; ++i) {
    in >> k;
    arr[i].resize(k);
    vis[i] = vector<int>(k, 0);
    for (auto &x : arr[i]) in >> x;
  }
  for (int i = 1; i < n; ++i) {
    for (int k = 0; k < (int)arr[i - 1].size(); ++k) {
      if (arr[i - 1][k].check_v()) arr[i - 1][k].fly();
      if (!arr[i - 1][k].check()) vis[i - 1][k] = 1;
    }
    for (int j = 0; j < (int)arr[i].size(); ++j) {
      for (int k = 0; k < (int)arr[i - 1].size(); ++k) {
        if (vis[i - 1][k]) continue;
        if (arr[i - 1][k].check_v()) {
          ...
          if (arr[i][j].match(arr[i - 1][k])) {
            arr[i][j].get_v(arr[i - 1][k]);
            vis[i - 1][k] = 1;
            break;
          }
        }
  ...
  for (int i = n; i < n + 10; ++i) {
    ...
    for (int k = 0; k < (int)arr[i - 1].size(); ++k) {
      if (arr[i - 1][k].check_v()) arr[i - 1][k].fly();
      if (!arr[i - 1][k].check()) continue;
      arr[i].emplace_back(arr[i - 1][k]);
    }
    print(arr[i]);
  }
}
